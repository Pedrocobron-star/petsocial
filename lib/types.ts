...
// ============================================================================
// Pet Avatar Config (avatar customizado SVG)
// ============================================================================
export type AvatarSpecies = 'dog' | 'cat' | 'rabbit' | 'bird' | 'fish' | 'other';

export type AvatarHeadShape = 'round' | 'oval' | 'wide' | 'long' | 'egg' | 'square';

export type AvatarEars =
  | 'pointed_up'    // gato/raposa
  | 'pointed_side'  // shiba/akita
  | 'droopy'        // beagle/cocker
  | 'long_droopy'   // basset/dachshund
  | 'round'         // urso/coelho longo
  | 'tall_round'    // mickey/rato
  | 'tiny'          // pug/persa
  | 'bat'           // bulldog francês — orelhas grandes pontudas pra cima
  | 'fluffy_down'   // shih tzu / yorkshire — orelhas pendentes peludas
  | 'cropped'       // doberman — pontudas curtas
  | 'none';
...
export type AvatarMouth =
  | 'smile'
  | 'neutral'
  | 'tongue_side' // língua pendurada
  | 'tongue_open' // de boca aberta
  | 'open'        // arfando
  | 'tiny'        // boquinha pequena
  | 'kiss';       // beijinho

export type AvatarPattern =
  | 'solid'
  | 'spots'       // pintas grandes
  | 'patches'     // manchas grandes (dálmata invertido)
  | 'stripes'     // tigrado / brindle
  | 'mask'        // máscara escura ao redor dos olhos
  | 'belly'       // ventre/peito mais claro
  | 'tuxedo'      // smoking
  | 'wrinkles'    // pug / bulldog — rugas no rosto
  | 'long_hair'   // shih tzu / maltês / yorkshire — cabelo longo cobrindo
  | 'mane'        // pomerânia / chow chow — pelo fluffy ao redor da cabeça
  | 'tricolor'    // beagle / pastor — 3 cores em zonas
  | 'saddle';     // pastor alemão — manto preto nas costas
...
/** Peito/corpo visível abaixo da cabeça (parte superior do tronco). */
export type AvatarChest =
  | 'none'      // sem peito visível
  | 'solid'     // peito da cor da pelagem
  | 'white'     // peito branco (clássico Border, Husky)
  | 'cream'     // peito creme/claro
  | 'spotted'   // pintas no peito (Dálmata)
  | 'fluffy';   // peito peludo (Pomerânia, Chow)

/** Rabo aparecendo no canto inferior direito do avatar. */
export type AvatarTail =
  | 'none'
  | 'bushy'     // Husky, Pastor — rabo grosso peludo
  | 'curly'     // Pug, Akita — enrolado pra cima
  | 'long'      // Golden, Lab — longo reto
  | 'pointed'   // Dálmata, Doberman — fino e reto
  | 'cropped';  // Boxer, Doberman, Pinscher — coto curto

/** Modificador de tamanho relativo (Chihuahua vs Rottweiler). 1.0 = padrão. */
export type AvatarSizeMod = 0.85 | 0.88 | 0.9 | 0.92 | 0.95 | 1.0 | 1.05 | 1.08 | 1.15;

/**
 * Adorno de cabelo extra — diferencia raças peludas similares:
 *  - 'topknot': nó/cocada no topo (Yorkshire, Shih Tzu de show)
 *  - 'fringe': franja caída sobre os olhos (Shih Tzu, Lhasa, Bichon)
 *  - 'parted': cabelo repartido no meio (Maltês de exposição)
 *  - 'shaggy': pelos desgrenhados extras (Border, Old English)
 */
export type AvatarHairAccent = 'topknot' | 'fringe' | 'parted' | 'shaggy';

/**
 * Cenário de fundo (substitui background_color quando definido).
 * Exclusivo do plano Pet Pro — adiciona ambiente ao avatar.
 */
export type AvatarBackgroundScene =
  | 'park'        // gradiente verde + sol
  | 'beach'       // gradiente amarelo/azul
  | 'night'       // azul escuro + estrelas
  | 'birthday'    // confetes
  | 'snow'        // azul claro + flocos
  | 'sunset';     // gradiente laranja/roxo

/**
 * Decoração pendurada na coleira (charm/pingente).
 * Exclusivo do plano Pet Pro.
 */
export type AvatarCollarCharm =
  | 'none'
  | 'bell'      // sino dourado
  | 'tag'       // plaquinha (formato osso)
  | 'heart'     // coraçãozinho vermelho
  | 'bone'      // ossinho branco
  | 'star'      // estrela dourada
  | 'diamond'   // diamante azul
  | 'paw';      // patinha
...
export interface PetAvatarConfig {
  ...
  /** Cor principal da pelagem (hex). */
  fur_color: string;
  /** Cor secundária pra padrões (spots, patches, stripes, mask, belly). */
  accent_color: string;
  /** Cor dos olhos (esquerdo). */
  eye_color: string;
  /** Cor do olho direito — quando definida e diferente, ativa heterocromia (Husky/Aussie). */
  ...
  // Novos campos (opcionais — undefined vira default no renderer)
  /** Peito/tronco visível abaixo da cabeça. Default 'solid'. */
  chest?: AvatarChest;
  /** Rabo no canto. Default 'none'. */
  tail?: AvatarTail;
  /** Multiplicador de escala (0.85 a 1.15). Default 1.0. */
  size_mod?: AvatarSizeMod;
  /** Adorno extra de cabelo (franja, topknot, etc). */
  hair_accent?: AvatarHairAccent;
  /** Pingente pendurado na coleira (sino, plaquinha, etc). Pro-only. */
  collar_charm?: AvatarCollarCharm;
  /** Cenário de fundo (parque, praia, etc) — substitui background_color. Pro-only. */
  background_scene?: AvatarBackgroundScene;
}
